...
namespace MimicRender
{
	...
	struct DirectLight : Light
	{
		...
		/// <summary> Get the ID of the depth map stores within the light. This value can only be assigned by a
		/// ShadowMapper::RenderDirectLightDepthMaps() call. </summary>
		unsigned int GetDepthMapTextureId() const noexcept;

		/// <summary> Get the 4x4 light matrix used to render the scene from the light's prespective for shadow
		/// mapping. Calculated by multiplying the lightProjection and lightView matricies together. </summary>
	...
	struct PointLight : Light
	{
		...
		/// <summary> Gets the constant term for the light attenuation formula. </summary>
		float GetConstant() const noexcept;

		/// <summary> Gets the linear term for the light attenuation formula. </summary>
		float GetLinear() const noexcept;

		/// <summary> Gets the quadratic term for the light attenuation formula. </summary>
